"""
Class which contain all graphic components and actions
# TODO : dispatch functionalities in different class
"""
...
from vtk.qt.QVTKRenderWindowInteractor import QVTKRenderWindowInteractor    # an import error is display but it work
...
class MainWindow(QtWidgets.QMainWindow, QtWidgets.QApplication):
    """
    A class which manage the application play the role of View and Controller in the MVC architecture
    """

    def __init__(self, app):
        """
        Constructor, build IHM
        :param app: the Qt QApplication
        """
        self.app = app
        ...
        # correspondence table use to link 3D visualization object et model
        # example: {3DNodeActor1=nodeObject1, 3DNodeActor2=nodeObject2}
        self.correspondence_table_nodes = {}
        self.correspondence_table_edges = {}

        self.renderer, self.frame, self.vtk_widget, self.interactor, self.render_window = self.setup()
        self.silhouette = setup_graph_selection(self)
        self.brain = setup_brain(self.renderer, self.app.BRAIN_FILE)
        self.mask = setup_mask(self.renderer, self.app.MASK_FILE)
        self.orientation_marker = setup_orientation_marker(self.interactor)

        # self.sphere = setup_sphere(self.renderer, self.interactor)
        # self.sphere2 = setup_sphere2(self.renderer, self.interactor)
        # self.sphere3 = setup_sphere3(self.renderer, self.interactor)
        # self.sphereWidget = setup_sphereWiget(self.interactor)

        # draw nodes
        # TODO : replace fixe data by loaded nodes from files
        nodes_coord = ((0, 0, 0), (50, 0, 0), (0, 50, 0), (0, 0, 50))
        for (x, y, z) in nodes_coord:
            new_node_actor = setup_node(self.renderer, x, y, z, 6.0)
            self.correspondence_table_nodes[new_node_actor] = (x, y, z)

        # draw edges
        # TODO : replace fixe data by loaded edges from files
        edges_coord = ((nodes_coord[0], nodes_coord[1]), (nodes_coord[0], nodes_coord[2]), (nodes_coord[1], nodes_coord[3]))
        for (p1, p2) in edges_coord:
            new_edge_actor = setup_edge(self.renderer, p1[0], p1[1], p1[2], p2[0], p2[1], p2[2], 6)
            self.correspondence_table_edges[new_edge_actor] = (p1, p2)

        # DRAW GRAPHIC INTERFACE
    ...
    def create_new_picker(max_value, min_value, step, picker_value, value_changed_func):
        ...
        :param max_value: the maximum value for the picker
        :param min_value: the minimum value for the picker
        :param step:  the minimum step available to change the picker value
        :param picker_value: the current value for the picker
        :param value_changed_func: the method to call when the picker have a state change
    ...
    def event_edge_selected(self, edge):
        """
        Function call by the silhouette when a edge is selected
        :param node: a edge object
        """
        # TODO
